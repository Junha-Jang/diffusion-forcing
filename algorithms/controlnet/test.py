...
# model = create_model('./models/control_v11p_sd21_canny.yaml').cpu()
# model.load_state_dict(load_state_dict('./models/control_v11p_sd21_canny.ckpt', location='cuda'))
model = model.cuda()
...
def process(
        input_image,
        control,
        prompt = 'blured mincraft screenshot',
    ...
    with torch.no_grad():
        ...
        # print(f'Detected map shape: {detected_map.shape}')
        # detected_map = HWC3(detected_map)

        # detected_map = np.random.randint(0, 255, (512, 512, 3)).astype(np.uint8)
        ...
        # print(f'Control tensor device: {control.device}')
